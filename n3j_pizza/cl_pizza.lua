...
function StworzBlip()
	...
	BlipPizza = AddBlipForCoord(283.79, -969.24, 29.87)
		...
		AddTextComponentString('Pizza u Masona')
...
function CreateMissionBlip(x,y,z)
	...
	AddTextEntry('MYBLIP', "Miejsce dostawy")
...
function WezDostawe()
	...
	exports['mythic_notify']:DoCustomHudText('inform', 'Zacząłeś dostawę w pizzeri, odnajdź lokalizacje na swoim GPS', 5000)
		...
		for k,v in pairs(Config.PrzygotowywaniePizzy) do
			local coords = GetEntityCoords(GetPlayerPed(-1))
				...
				if not inMission then
					DrawText3D(v.x, v.y, v.z, ('[E] aby ~g~ przygotować pizzę'))	
					if IsControlJustReleased(0, 46) and (GetDistanceBetweenCoords(coords, v.x, v.y, v.z, true) < 0.3) then
						FreezeEntityPosition(PlayerPedId(), true)
								...
								local finished = exports["gen-taskbar"]:taskBar(10000,"Przygotowywanie pizzy")
								Citizen.Wait(1000)
	...
	Citizen.Wait(0)
		for k,v in pairs(Config.PakowaniePizzy) do
			local coords = GetEntityCoords(GetPlayerPed(-1))
				if (GetDistanceBetweenCoords(coords, v.x, v.y, v.z, true) < 3.0)  then
				if not inMission then
					DrawText3D(v.x, v.y, v.z, ('[E] aby ~g~ włożyć do pieca i zapakować ~w~pizzę'))	
					if IsControlJustReleased(0, 46) and (GetDistanceBetweenCoords(coords, v.x, v.y, v.z, true) < 0.3) then
						FreezeEntityPosition(PlayerPedId(), true)
						ClearPedSecondaryTask(PlayerPedId())
						loadAnimDict( "mini@repair" ) 
						TaskPlayAnim( PlayerPedId(), "mini@repair", "fixing_a_ped", 8.0, 1.0, -1, 16, 0, 0, 0, 0 )
                        local finished = exports["gen-taskbar"]:taskBar(9700,"Pakowanie pizzy do pudełka")
						Citizen.Wait(1850)
						FreezeEntityPosition(PlayerPedId(), false)
						ClearPedTasks(PlayerPedId())
	...
	Citizen.Wait(0)
		for k,v in pairs(Config.JobPoints) do
			local coords = GetEntityCoords(GetPlayerPed(-1))
				if (GetDistanceBetweenCoords(coords, v.x, v.y, v.z, true) < 3.0)  then
				if not inMission then
					DrawText3D(v.x, v.y, v.z, ('[E] aby ~g~dostarczyć ~w~pizzę'))	
					...
					DrawText3D(v.x, v.y, v.z, ('[E] aby ~r~przerwać ~w~dostawę pizzy'))	
					if IsControlJustReleased(0, 46) and (GetDistanceBetweenCoords(coords, v.x, v.y, v.z, true) < 0.5) then
						ClearPedSecondaryTask(PlayerPedId())
						loadAnimDict( "anim@heists@keycard@" ) 
						TaskPlayAnim( PlayerPedId(), "anim@heists@keycard@", "exit", 8.0, 1.0, -1, 16, 0, 0, 0, 0 )
						...
						ClearPedTasks(PlayerPedId())
						inMission = false
						exports['mythic_notify']:DoHudText('error', 'Przerwałeś dostawę pizzy')
					end
				end
		end
	end
...
Citizen.CreateThread(function()
	...
	local d = GetDistanceBetweenCoords(pcoords, coords[1], coords[2], coords[3], true)
		if inMission == true and MissionCoords ~= false and d < 20.0 then
			DrawText3D(coords[1], coords[2], coords[3], '[E] aby ~g~ dostarczyć ~w~Pizzę')
			if IsControlJustReleased(0, 38) and d < 5.0 then
				...
				local finished = exports["gen-taskbar"]:taskBar(3500,"Pukanie do drzwi")
